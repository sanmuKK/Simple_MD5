...
public:
    ...
    MD5(const string& message, int isfile);
...
MD5::MD5(const string& message, int isfile) {
    finished = false;
...
void MD5::fileinit(string filename) {

    FILE* fin;
    fopen_s(&fin, filename.c_str(), "rb");
    unsigned long long len = file_size(filename.c_str());
    if (!fin) {
        cout << "Can not open " << filename << endl;
        exit(0);
    }
    bit i, index, partLen, j;
    ...
    partLen = 64 - index;
    if (len >= partLen) {
        fread(fileinput, partLen, 1, fin);
        memcpy(&buffer[index], fileinput, partLen);
        transform(buffer);
        for (j = partLen; unsigned long long(j) + 40959 < len; j += 40960)
        {
            fread(fileinput, 40960, 1, fin);
            for (i = 0; i < 40960; i += 64) {
                filetransform(&fileinput[i]);
            }
        }
        index = 0;
    }
    ...
    fread(fileinput, len - j, 1, fin);
    for (i = 0; unsigned long long(i) + 63 < len - j; i += 64) {
        filetransform(&fileinput[i]);
    }
    memcpy(&buffer[index], &fileinput[i], len - j - i);
}
...
void MD5::filetransform(char block[64]) {
...
void MD5::filegetX(char* input, bit* output, size_t length) {
    for (size_t i = 0, j = 0; j < length; ++i, j += 4) {
        output[i] = ((bit)input[j]) | (((bit)input[j + 1]) << 8) |
            (((bit)input[j + 2]) << 16) | (((bit)input[j + 3]) << 24);
    }
...
void printMD5(string& message) {
    ...
    cout <<"MD5结果(32位)：" << ans32 << endl;
    string ans16 = ans32.substr(8, 16);
    cout << "MD5结果(16位)：" << ans16 << endl;
}

void printfileMD5(string& filename) {
    string ans32 = MD5(filename, 1).toStr();
    cout << "MD5结果(32位)：" << ans32 << endl;
    string ans16 = ans32.substr(8, 16);
    cout << "MD5结果(16位)：" << ans16 << endl;
}
...
int main() {
    int choice = 0;
    cout << "字符串MD5输入1，文件MD5输入2：" << endl;
    cin >> choice;
    if (choice == 1)
    {
        string str;
        cout << "请输入待MD5的字符串：" << endl;
        cin >> str;
        printMD5(str);
    }
    else if (choice == 2)
